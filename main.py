...
async def send_welcome(message: types.Message):
    ...
    await message.answer("Привет, солнце! ☀️ Ты в таймере по методу суперкомпенсации. Кожа адаптируется к солнцу постепенно — и загар становится ровным, глубоким и без ожогов. Начинай с шага 1. Даже если уже немного загорел(а), важно пройти путь с начала. Каждый новый день и после перерыва — возвращайся на 2 шага назад. Хочешь разобраться подробнее — жми ℹ️ Инфо. Там всё по делу.", reply_markup=get_step_buttons())
...
async def send_info(message: types.Message):
    await message.answer(
        ...
        "С 11:00 до 17:00 — солнце более агрессивное. "
...
async def handle_step(message: types.Message):
    ...
    if step:
        await message.answer(f"Шаг {step['step']} — {step['duration_min']} минут. Следи за временем и положением тела. Если был перерыв — начни с шага “минус два” от последнего.")
